	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if($atts['type'] == 'none' || (empty($atts['title']) && empty($atts['caption']))){
				if( !empty($atts['carousel']) ){
					$ret .= '<i class="icon-angle-left gdlr-flex-prev fa fa-angle-left"></i>';
					$ret .= '<i class="icon-angle-right gdlr-flex-next fa fa-angle-right"></i>';
				}else if( !empty($atts['additional_html']) ){
					$ret .= $atts['additional_html'];
				}
			}else if($atts['type'] == 'center'){
				$ret .= !empty($atts['carousel'])? '<i class="icon-angle-left fa fa-angle-left gdlr-flex-prev"></i>': '';
				$ret .= '<h3 class="gdlr-item-title gdlr-skin-title gdlr-skin-border">' . $atts['title'] . '</h3>';
				$ret .= !empty($atts['carousel'])? '<i class="icon-angle-right fa fa-angle-right gdlr-flex-next"></i>': '';
	
				if( !empty($atts['caption']) ){
					$ret .= '<div class="clear"></div>';
					$ret .= '<div class="gdlr-item-caption gdlr-skin-info">' . $atts['caption'] . '</div>';
				}	
				$ret .= empty($atts['additional_html'])? '': $atts['additional_html'];
			}else if($atts['type'] == 'icon'){
				$ret .= !empty($atts['carousel'])? '<i class="icon-angle-left gdlr-flex-prev fa fa-angle-left"></i>': '';
				$ret .= !empty($atts['icon'])? '<i class="gdlr-item-title-icon fa ' . $atts['icon'] . '"></i>': '';					
				$ret .= '<h3 class="gdlr-item-title gdlr-skin-title gdlr-skin-border">' . $atts['title'] . '</h3>';
				$ret .= $atts['additional_html'];
				$ret .= !empty($atts['carousel'])? '<i class="icon-angle-right gdlr-flex-next fa fa-angle-right"></i>': '';
			}else{
				$ret .= !empty($atts['icon'])? '<i class="gdlr-item-title-icon fa ' . $atts['icon'] . '"></i>': '';						
				$ret .= '<h3 class="gdlr-item-title gdlr-skin-title gdlr-skin-border">' . $atts['title'] . '</h3>';
				$ret .= $atts['additional_html'];
				...
				if( !empty($atts['carousel']) ){
					$ret .= '<div class="gdlr-nav-carousel" >';
					$ret .= '<i class="icon-angle-left gdlr-flex-prev fa fa-angle-left"></i>';
					$ret .= '<i class="icon-angle-right gdlr-flex-next fa fa-angle-right"></i>';
					$ret .= '</div>';
				}
			...
			if( !empty($ret) ){
				$ret = '<div class="gdlr-item-title-wrapper pos-' . $atts['type'] . ' ' . $atts['additional_class'] . '">' . $ret;
				$ret .= '<div class="clear"></div>';
				$ret .= '</div>';			
			}
	...
	if( !function_exists('gdlr_get_column_service_item') ){
		function gdlr_get_column_service_item( $settings ){
			...
			if( $settings['style'] == 'type-image' ){
				$ret .= '<div class="column-service-image">';
	...
	// skill round item
	if( !function_exists('gdlr_get_skill_round_item') ){
		function gdlr_get_skill_round_item( $settings ){	
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$ret  = '<div class="gdlr-round-skill-bar-wrapper gdlr-item" ' . $item_id . $margin_style . '>';
			$ret .= '<div class="gdlr-round-skill-content" style="color:' . $settings['text-color'] . ';">';
			$ret .= '<div class="gdlr-round-skill-title">' . $settings['title'] . '</div>';
			$ret .= '<div class="gdlr-round-skill-caption">' . $settings['caption'] . '</div>';
			$ret .= '</div>'; // gdlr-round-skill-content
			$ret .= '</div>'; // gdlr-round-skill-bar-wrapper				
		...
		function gdlr_get_price_table_item( $settings ){
			...
			foreach( $list as $tab ){ 
				...
				if(!empty($tab['gdl-tab-link'])){
					...
					$ret .= '<a class="gdlr-button without-border" href="' . $tab['gdl-tab-link'] . '">' . __('Buy Now', 'gdlr_translate') . '</a>';
					$ret .= '</div>';
				}
	...
	if( !function_exists('gdlr_get_stunning_text_item') ){
		function gdlr_get_stunning_text_item( $settings ){
			...
			if( !empty($settings['button-link']) ){
				$ret .= '<a class="stunning-text-button gdlr-button" href="' . $settings['button-link'] . '" target="_blank" >';
				$ret .= $settings['button-text'];
				$ret .= '</a>';
			}
			$ret .= '</div>'; // gdlr-item
	...
	if( !function_exists('gdlr_get_styled_box_item') ){
		function gdlr_get_styled_box_item( $settings ){
			...
			if( !empty($settings['icon']) ){
				$ret .= '<div class="gdlr-styled-box-icon" ><i class="fa ' . gdlr_text_filter($settings['icon']) . '" ></i></div>';
			}			
			if( !empty($settings['caption']) ){
				$ret .= '<div class="gdlr-styled-box-caption" >' . gdlr_text_filter($settings['caption']) . '</div>';
			}
			if( !empty($settings['title']) ){
				$ret .= '<h3 class="gdlr-styled-box-title" style="color: ' . $settings['content-color'] . ';">';
				$ret .= gdlr_text_filter($settings['title']) . '</h3>';
			}
			$ret .= gdlr_content_filter($settings['content']);
	...
	if( !function_exists('gdlr_get_static_testimonial_item') ){
		function gdlr_get_static_testimonial_item( $settings ){	
			...
			if( $settings['title-type'] == 'quote' ){
				$settings['title-type'] = 'center';
				$settings['title'] = '<i class="icon-quote-left fa fa-quote-left"></i>';
				$settings['caption'] = '';
			}
			...
			foreach( $list as $tab ){ 
				...
				// author image for plain style
				if( strpos($settings['testimonial-style'], 'plain-style') !== false && !empty($tab['gdl-tab-author-image']) ){
				...
				if( strpos($settings['testimonial-style'], 'plain-style') === false ){
					$ret .= '<div class="testimonial-author-image gdlr-skin-border" >';
					$ret .= gdlr_get_image($tab['gdl-tab-author-image'], 'thumbnail');
					$ret .= '</div>';
	...
	if( !function_exists('gdlr_get_carousel_testimonial_item') ){
		function gdlr_get_carousel_testimonial_item( $settings ){	
			...
			$ret .= gdlr_get_item_title(array(
				...
				'type' => $settings['title-type'],
				'carousel' => true,
				'additional_class' => 'gdlr-nav-container'
			));							
	...
	if( !function_exists('gdlr_get_classic_page_list') ){
		function gdlr_get_classic_page_list($query, $size, $thumbnail_size, $layout = 'fitRows'){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<span class="portfolio-icon" ><i class="icon-link fa fa-link" ></i></span>';
				$ret .= '</a>';	
