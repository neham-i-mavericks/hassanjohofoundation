...
function backup_db_filesize_formatted( $size ) {
    $units = array( 'B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB');
    $power = $size > 0 ? floor(log($size, 1024)) : 0;
    return number_format($size / pow(1024, $power), 2, '.', ',') . ' ' . $units[$power];
}

function backup_db_flatToTree($flat) {
    $res = array();
    foreach ($flat as $e) {
        $parent =& $res;
...
function backup_db_listDirectoryByFolder($path) {
    $path = rtrim($path, '/') . '/*';
...
function backup_db_delTree($dir) { 
   $files = array_diff(scandir($dir), array('.','..')); 
    foreach ($files as $file) { 
      (is_dir("$dir/$file")) ? backup_db_delTree("$dir/$file") : unlink("$dir/$file"); 
    } 
    return rmdir($dir); 
  } 
