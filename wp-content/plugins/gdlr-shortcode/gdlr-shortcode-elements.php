<?php
	...
	function gdlr_process_shortcode( $atts, $content = null ){
		...
		foreach( $gdlr_shortcode_tabs as $tab ){
			...
			if( $item_num > 1 ){
				...
				$ret .= ($type == 'horizontal')? '<div class="icon-chevron-right fa fa-chevron-right" ></div>': '<div class="icon-chevron-down fa fa-chevron-down" ></div>';
				$ret .= '</div>';
			}
	...
	function gdlr_button_shortcode( $atts, $content = null ){
		...
		$button_class  = $size;
		$button_class .= ($with_border == 'no')? ' without-border': ''; 
	...
	function gdlr_image_link_shortcode( $atts ){
		...
		$link_attr = ''; $link_icon = '';
		if($type == 'video'){
			$link_icon = 'icon-play fa fa-play';
			$link_attr = ' data-rel="fancybox" data-fancybox-type="iframe" ';
		}else if($type == 'image'){
			$link_icon = 'icon-search fa fa-search';
			$link_attr = ' data-rel="fancybox" ';
		}else{
			$link_icon = 'icon-link fa fa-link';
		}
		...
		$ret .= '<span class="gdlr-image-link-icon"><i class="' . $link_icon . '" ></i></span>';
		$ret .= '</a>';
		$ret .= '</div>';
		return $ret;
	}	
	...
	function gdlr_icon_shortcode($atts, $content = null){
		...
		$ret .= '<i class="gdlr-icon ' . $type . '" ' . $style . ' ></i>';
		$ret .= (empty($link))? '': '</a>';
		return $ret;
	}		
	...
	// video shortcode
	add_shortcode('gdlr_paypal', 'gdlr_paypal_shortcode');
	function gdlr_paypal_shortcode($atts, $content = null){
		return gdlr_paypal_form($atts);
	}	
